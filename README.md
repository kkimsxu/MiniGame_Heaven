...
- **김현준**
  - 테트리스 게임
    - 블럭이 바닥에 닿을 경우 하강을 멈춰 블럭이 정지하게 하였고, 플레이어가 방향키 입력을 통해 블럭을 움직이더라도 게임 스크린을 벗어날 수 없게 수정했습니다.
    - 게임 스크린을 벗어나지 못하게 하는 부분에서 오른쪽 벽의 경우 의도대로 블럭이 벽에 닿으면 움직이거나 회전할 수 없지만 왼쪽 벽의 경우 움직이거나 회전이 가능한 버그가 있어 차후에 수정할 예정입니다.
 
- **최윤형**
  - 슬라이드 퍼즐 게임
    - 게임의 재미를 위해 숫자 대신 그림으로 변경하여 게임의 난이도를 상승시켰습니다.
    - 최종적으로 오류가 등장하는지 마지막으로 검토하고 게임을 완성했습니다.

- **김지수**
  - 구슬 게임
  - 2주차에서 구슬 게임을 완성하여 기본적인 플레이가 가능한 상태로 만들었습니다. 3주차에는 게임에서 발생할 수 있는 다양한 오류를 방지하기 위해 여러 번 데모를 돌려봤습니다.
    여러 번의 데모를 통해 발견된 오류나 예외 상황이 없어서, 코드를 그대로 유지하였습니다.


### 4주차
- 최종 완성
- 사용자가 원하는 게임 버튼을 클릭하면 해당 게임이 즉시 시작되도록 각 게임의 모듈을 통합하여 미니게임천국을 완성했습니다. 
