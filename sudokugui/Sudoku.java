...
public class Sudoku {
	...
	public boolean check(int digit, int row, int col) {
		if (solution[row][col] == digit) {
			puzzle_board[row][col] = digit; // 정답인 경우 숫자 저장
			return true;
		} else {
			puzzle_board[row][col] = -digit; // 정답이 아닌 경우 부정적인 값으로 저장
			return false;
		}
	}
	...
	public boolean isCompleted() {
		for (int i = 0; i < 9; i++) {
			for (int j = 0; j < 9; j++) {
				if (puzzle_board[i][j] == 0) {
					return false; // 빈 칸이 있으면 게임이 완료되지 않음
				}
			}
		}
		return true; // 모든 칸이 채워졌으면 게임 완료
	}
